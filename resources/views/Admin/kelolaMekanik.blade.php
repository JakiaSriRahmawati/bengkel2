...
<body>
    <div class="sidebar">
        ...
        <ul class="nav flex-column">
            ...
            <li class="nav-item mb-2">
                <a class="nav-link active" href="{{ route('kelolaMekanik') }}">
        ...
        <div class="card shadow-sm p-3 b-5 bg-body rounded">
            <div class="card-body">
                ...
                <div class="d-flex mb-3">
                    <form action="{{ route('kelolaMekanik') }}" method="GET" class="d-flex me-2">
                        <input type="search" id="search" name="search" class="input form-control rounded me-2" placeholder="Cari Mekanik">
                        <button type="submit" class="btn btn-primary">Cari</button>
                    </form>
                    <button class="btn btn-primary ms-auto align-self-start rounded" data-bs-toggle="modal" data-bs-target="#tambahMekanikModal">Tambah Mekanik</button>
                </div>
                    ...
                    <table class="table table-striped table-bordered" id="example">
                        ...
                        <tbody class="text-center">
                            @foreach ($m as $item)
                                <tr>
                                    <td>{{ $item->id }}</td>
                                    <td><img src="{{ asset('storage/' . $item->profil) }}" alt="Profil" width="50"></td>
                                    <td>{{ $item->nama }}</td>
                                    <td>{{ $item->email }}</td>
                                    <td>{{ $item->alamat }}</td>
                                    <td>{{ $item->kontak }}</td>
                                    ...
                                    <td class="actions">
                                        ...
                                        <a href="{{ route('hapusUser', $item->id) }}" class="btn btn-outline-danger"
                                            onclick="return confirm('Yakin Mau Hapus?')">Hapus <i class="bi bi-trash2-fill"></i></a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
